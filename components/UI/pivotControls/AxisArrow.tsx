...
import { metersToInches } from "@/utils/getActiveAxis";
...
export const AxisArrow: React.FC<{
  ...
  axis: 0 | 1 | 2;
}> = ({ snapTranslate = 1, direction, axis }) => {
  const {
    translation,
    translationLimits,
    annotationsClass,
  ...
  const onPointerMove = React.useCallback(
    (e: ThreeEvent<PointerEvent>) => {
      ...
      if (clickInfo.current) {
        ...
        if (e.shiftKey) {
          offset = Math.round(offset / snapTranslate) * snapTranslate;
        }
        translation.current[axis] = offset0.current + offset;
        if (displayValues) {
          let val = translation.current[axis];
          if (Inches) {
            // Display in inches
            val = metersToInches(translation.current[axis]);
            divRef.current.innerText = `${val.toFixed(2)} in`;
          } else
            divRef.current.innerText = `${translation.current[axis].toFixed(
              2
            )} m`;
        }
