    ...
    const config = React.useMemo(
      () => ({
        ...
        onDragEnd: (mdW: THREE.Matrix4) => {
          if (onDragEnd && Bool) onDragEnd(mL, mdL, mW, mdW);
          Bool = false;
          invalidate();
        },
        translation,
      ...
      <context.Provider value={config}>
        <group ref={parentRef}>
          <group ref={ref} matrix={matrix} matrixAutoUpdate={false}>
            {/* <group ref={ref} position={[translateP[0],translateP[1],translateP[2]]} rotation={Rotation} scale={[1,1,1]} > */}
            <group
              visible={visible}
              ref={gizmoRef}
              position={offset}
            ...
            >
              ...
              {!disableScaleAxes && activeAxes[0] && (
                <SphereScale axis={0} direction={xDir} />
              )}
              {!disableScaleAxes && activeAxes[1] && (
                <SphereScale axis={1} direction={yDir} />
              )}
              {!disableScaleAxes && activeAxes[2] && (
